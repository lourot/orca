...
## Project header labels and order

Project headers read `<parent-folder>/<name>` — `enpal/orca`, not `orca` — because the folder
above a checkout is usually what tells two same-named repos apart. Headers expand further
(`grandparent/parent/name`) when that is still ambiguous. The label is truncated to fit;
hover it for the full string. Other surfaces, including the `Cmd-J` repository filter chips,
keep the plain repo name.

Sidebar options → **Project order** chooses how those headers are ordered:

- **Manual** — your drag order. The only mode in which repo rows can be dragged.
- **Recent** — most recent workspace activity first.
- **Name** — alphabetical on the header as displayed, so `enpal/orca` files under "e".

Project groups are not affected: their headers keep the order of the tab bar.
