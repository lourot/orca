...
A wake is always a resume of the **same** session id, never a branch — if you want a second agent working from the same history, [fork the session](/docs/terminal#fork-an-agent-session) instead, which copies it under a new id.
