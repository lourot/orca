...
Orca reads Codex rate limits through the active account's existing ChatGPT/SSO login and surfaces them in the status bar. When the provider reports a monthly credit limit, the Usage surface shows the used credits, limit, and reset time for the **Active account**. If Codex Usage Tracking is enabled and the provider does not report monthly credits, Orca may show a clearly labeled **Local accounts · estimated** fallback based on local rollout logs. See [Usage & rate-limit tracking](/docs/agents/usage-tracking).
