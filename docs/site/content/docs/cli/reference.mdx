...
`file open` also takes an absolute path that belongs to no workspace, on a local workspace only — the
selected workspace hosts the tab, and the file is editable and saves back to disk. When cwd is inside
no workspace, the workspace focused in Orca hosts it and a relative path resolves against cwd:

```bash
cd ~ && orca file open .zshrc
```
