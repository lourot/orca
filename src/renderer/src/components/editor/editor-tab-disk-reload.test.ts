...
describe('requestEditorTabDiskReload', () => {
  ...
  it('confirms a clean-tab reload with a toast that offers no undo', () => {
    mockStore([makeFile()])

    requestEditorTabDiskReload('file-1')

    expect(dispatchedFileIds).toEqual(['file-1'])
    // Nothing was discarded, so there is nothing to undo — but the reload is
    // otherwise silent when the disk content already matches.
  ...
  it('discards a dirty tab draft (with the undo toast) before dispatching', () => {
    ...
    expect(toastMock).toHaveBeenCalledTimes(1)
    expect(toastMock.mock.calls[0]?.[1]?.action).toBeDefined()
